...
int main(void)
{
    ...
    camera.target = (Vector3){ 10.0f, 10.0f, 1.0f };     // Camera looking at point
    ...
    Texture2D texture = LoadTexture("resources\wabbit_alpha.png"); // Load model texture
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if (IsKeyDown(KEY_Q))
        {
            camera.position.y += 3.0f * GetFrameTime();
        }
        if (IsKeyDown(KEY_E))
        {
            camera.position.y += -3.0f * GetFrameTime();
        ...
        DrawText("(tm) El modelo lo hizo Héctor", screenWidth - 200, screenHeight - 20, 10, GRAY);
